...
#define EXEC_WAIT_THC       bit(5) // bitmask 00100000
...
#define EXEC_DWELL_OVR_RESET         bit(0)
#define EXEC_DWELL_OVR_COARSE_PLUS   bit(1)
#define EXEC_DWELL_OVR_COARSE_MINUS  bit(2)
#define EXEC_DWELL_OVR_FINE_PLUS     bit(3)
#define EXEC_DWELL_OVR_FINE_MINUS    bit(4)

#define EXEC_DRY_RUN_TOGGLE          bit(7)
...
#define STATE_WAIT_THC      bit(6) // Waiting until THC reports that torch has fired.
...
#define SUSPEND_WAIT_THC          bit(5) // Tracks THC torch fired state for resuming.
...
typedef struct {
  ...
  uint16_t d_override;         // Dwell time override value in percent
...
extern volatile uint8_t sys_rt_exec_dwell_override; // Global realtime executor bitflag variable for dwell time overrides.
extern volatile uint8_t sys_rt_exec_dry_run; // Global realtime executor bitflag variable for dry run.

extern volatile bool sys_rt_exec_thc; // Global realtime executor bitflag variable for THC.
...
// Returns if THC reports that torch has fired, based on pin state.
...
// Set machine position of axis 'idx' to 'position'.
...
void system_set_exec_dwell_override_flag(uint8_t mask);
void system_set_exec_dry_run_flag(uint8_t mask);

void system_thc_enable(bool state);

void system_clear_exec_motion_overrides();
void system_clear_exec_accessory_overrides();
void system_clear_exec_dwell_overrides();
void system_clear_exec_dry_run();
