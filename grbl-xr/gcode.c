...
uint8_t gc_execute_line(char *line)
{
    ...
    switch(letter) {
      ...
      default:
        ...
        switch(letter){
          ...
          case 'H': // THC during program
              word_bit = WORD_H;
              switch(int_value){
                  case 0: gc_block.modal.thc = THC_ENABLE; break;
                  case 1: gc_block.modal.thc = THC_DISABLE; break;
              }
            break;
  ...
  */
  ...
  if (value_words) { bit_false(value_words,(bit(WORD_H))); } // Remove THC words.
  ...
  */
  ...
  // [7.1. THC Control ]:
  if (gc_state.modal.thc != gc_block.modal.thc) {
      protocol_buffer_synchronize();
    system_thc_enable(gc_block.modal.thc);
    gc_state.modal.thc = gc_block.modal.thc;
  }
//  pl_data->condition |= gc_state.modal.thc; // Set condition flag for planner use.
  ...
  if (gc_state.modal.program_flow) {
    ...
    if (gc_state.modal.program_flow == PROGRAM_FLOW_PAUSED) {
      ...
      gc_state.modal.coolant = COOLANT_DISABLE;
        ...
        sys.d_override = DEFAULT_DWELL_TIME_OVERRIDE; // Set to 100%
      #endif
