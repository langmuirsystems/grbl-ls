...
//    }
  ...
  rt_exec = sys_rt_exec_dwell_override; // Copy volatile sys_rt_exec_dwell_override
  if (rt_exec) {
    system_clear_exec_dwell_overrides(); // Clear all dwell time override flags.

    uint16_t new_d_override =  sys.d_override;
    if (rt_exec & EXEC_DWELL_OVR_RESET) { new_d_override = DEFAULT_DWELL_TIME_OVERRIDE; }
    if (rt_exec & EXEC_DWELL_OVR_COARSE_PLUS) { new_d_override += DWELL_OVERRIDE_COARSE_INCREMENT; }
    if (rt_exec & EXEC_DWELL_OVR_COARSE_MINUS) { new_d_override -= DWELL_OVERRIDE_COARSE_INCREMENT; }
    if (rt_exec & EXEC_DWELL_OVR_FINE_PLUS) { new_d_override += DWELL_OVERRIDE_FINE_INCREMENT; }
    if (rt_exec & EXEC_DWELL_OVR_FINE_MINUS) { new_d_override -= DWELL_OVERRIDE_FINE_INCREMENT; }
    new_d_override = min(new_d_override,MAX_DWELL_TIME_OVERRIDE);
    new_d_override = max(new_d_override,MIN_DWELL_TIME_OVERRIDE);

    if (new_d_override != sys.d_override) {
      sys.d_override = new_d_override;
      sys.report_ovr_counter = 0; // Set to report change immediately
    }
  }
   ...
   // DRY RUN
    rt_exec = sys_rt_exec_dry_run; // Copy volatile sys_rt_exec_dry_run
   if (rt_exec) {
       system_clear_exec_dry_run();
       if (!bit_istrue(settings.extended_flags,EXT_BITFLAG_DRY_RUN)) {
