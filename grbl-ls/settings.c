...
const __flash settings_t defaults = {\
    ...
    .extended_flags = (DEFAULT_DRY_RUN << EXT_BIT_DRY_RUN),
...
uint8_t settings_store_global_setting(uint8_t parameter, float value) {
  ...
  // Don't write dry run flag to EEPROM, as we never want the machine
  // to start in dry run mode.
  uint8_t saved_extended_flags = settings.extended_flags;
  #if DEFAULT_DRY_RUN
    settings.extended_flags |= EXT_BITFLAG_DRY_RUN;
  #else
    settings.extended_flags &= ~EXT_BITFLAG_DRY_RUN;
  ...
  write_global_settings();
  settings.extended_flags = saved_extended_flags;
  return(STATUS_OK);
}
