...
void system_probe_select(uint8_t value) {

    sys.probe_select = value;

    PROBE_CONTROL1_DDR |= PROBE_CONTROL1_MASK; //outputs
    PROBE_CONTROL2_DDR |= PROBE_CONTROL2_MASK;

    switch(value){
        case 0: // Either (both high)
            PROBE_CONTROL1_PORT |= PROBE_CONTROL1_MASK; //HIGH A3
            PROBE_CONTROL2_PORT |= PROBE_CONTROL2_MASK; //HIGH A5
            break;

        case 1: // Probe A
            PROBE_CONTROL1_PORT &= ~(PROBE_CONTROL1_MASK); //LOW A3
            PROBE_CONTROL2_PORT |= PROBE_CONTROL2_MASK; //HIGH A5
            break;

        case 2: // Probe B
            PROBE_CONTROL1_PORT |= PROBE_CONTROL1_MASK; //HIGH A3
            PROBE_CONTROL2_PORT &= ~(PROBE_CONTROL2_MASK); //LOW A5
            break;
    }
}