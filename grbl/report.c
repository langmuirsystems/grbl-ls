...
//          #endif
            ...
            if (bit_istrue(lim_pin_state,(bit(N_AXIS)))) { serial_write('Y'); serial_write('2'); }
          #endif
...
//          if (bit_istrue(lim_pin_state,bit(Z_AXIS))) { serial_write('Z'); }
  ...
  #endif
  // Report realtime Q status
  #ifdef REPORT_FIELD_Q_STATE
  printPgmString(PSTR("|Q:"));
  if (sys.probe_select == 0) { serial_write('0'); }
  if (sys.probe_select == 1) { serial_write('1'); }
  if (sys.probe_select == 2) { serial_write('2'); }
  #endif
