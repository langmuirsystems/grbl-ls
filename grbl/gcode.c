...
//        case 'H':
          ...
          case 'Q': // Probe Select during Canned Cycle
                 word_bit = WORD_Q;
                          switch(int_value){
                              case 0: gc_block.modal.probe_select = PROBE_SELECT; break;
                              case 1: gc_block.modal.probe_select = PROBE_SELECT_A; break;
                              case 2: gc_block.modal.probe_select = PROBE_SELECT_B; break;
                          }
                        break;
...
//              if (delta_r > (0.01*gc_block.values.r)) { FAIL(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.005mm AND 0.1% radius
            }
          }
          break;
  ...
  if (value_words) { bit_false(value_words,(bit(WORD_Q))); } // Remove Probe Select words.
...
// [7.2 Set Probe Selection]:
  if (gc_state.modal.probe_select != gc_block.modal.probe_select) {
      protocol_buffer_synchronize();
    system_probe_select(gc_block.modal.probe_select);
    gc_state.modal.probe_select = gc_block.modal.probe_select;
  }
